...
livedev:
	@echo Serving pages on $(SPHINX_URL)
	sphinx-autobuild -b html -t dev $(ALLSPHINXOPTS) $(BUILDDIR)/html --port=$(SPHINX_PORT) --host=$(SPHINX_HOST) > /dev/null
...
surge-html: html
	surge -d $(DOCS_DOMAIN).surge.sh -p build/html
surge-dev: dev
	surge -d dev.$(DOCS_DOMAIN).surge.sh -p build/dev
